...
resLasso1 <- lasso(S, R, X, Sigma, Nsample = Nsample_test, burn_in = burn_in_test)

library(bayesplot)
mcmc_trace(data.frame(resLasso1$Beta))


#plot Z's


Z_temp <- matrix(unlist(resLasso1$Z), ncol = N, byrow = T)
head(Z_temp, 10)
...
# Lamdba 
xx <- seq(min(resLasso1$Lambda2), max(resLasso1$Lambda2), length.out = Nsample_test)
yy <- dgamma(xx, 2, 1/2)


hist(resLasso1$Lambda2, freq = F)
lines(xx,yy, type = "l", col = "blue" )
lines(density(resLasso1$Lambda2), col = "red", lwd = 1.2)
legend("topright", 
