...
ridge.new <-  function(S, R, X, Sigma, Nsample = 1000, burn_in = 200, r = 5, a = 0.5, b = 20){
  ...
  lambda <- rep(0,Nsample+1)
  lambda[1] <- rgamma(1, shape = r, scale = delta[1])
  ...
  for (t in 1:Nsample){
    ...
    lambda[t+1] <- rgamma(1, shape = r+P/2, rate = delta[t] + sum(beta_ridge[t+1,]^2)/2)
    
    # 4. Generate delta 
    
    delta[t+1] <- rinvgamma(1, alpha = a+r, beta=lambda[t+1]+b)
