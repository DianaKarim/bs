...
lasso_res <- unlist(lapply(D:M, function(step){
  R <- c(R2[1:step],rep(2,M-step))
  res <- lasso(S, R, X, Sigma, Nsample = Nsample_test, burn_in = burn_in_test)
  mean(res$Beta)
}))

#plot(lasso_res, type = "l")

hs_res <- unlist(lapply(D:M, function(step){
  R <- c(R2[1:step],rep(2,M-step))
  res <- horseshoe(S, R, X, Sigma, Nsample = Nsample_test, burn_in = burn_in_test)
  mean(res$Beta)
}))

#plot(hs_res, type = "l", col = "red")

ridge_res <- unlist(lapply(D:M, function(step){
  R <- c(R2[1:step],rep(2,M-step))
  res <- ridge(S, R, X, Sigma, Nsample = Nsample_test, burn_in = burn_in_test)
  mean(res$Beta)
}))

plot(ridge_res, type = "l", col = "blue")


# Improper
imp_res <- unlist(lapply(D:M, function(step){
  R <- c(R2[1:step],rep(2,M-step))
  res <- imp(S, R, X, Sigma, Nsample = Nsample_test, burn_in = burn_in_test)
  mean(res$Beta)
}))

#plot(imp_res, type = "l", col = "blue")
...
cl<-makeCluster(3)
clusterExport(cl, c("R2", "S", "R", "X", "D", "M", "imp", "lasso", "ridge", "horseshoe", "Nsample_test", 
